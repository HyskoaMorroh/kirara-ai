...
def write_project_data(data_path: Path, marker: str, config: str | None = None) -> None:
    ...
    (data_path / "memory" / "state.json").write_text(json.dumps({"marker": marker}), encoding="utf-8")
    (data_path / "media" / "files" / "example.txt").write_text(marker, encoding="utf-8")
    (data_path / "db" / "kirara.db").write_text(marker, encoding="utf-8")
    (data_path / "plugins" / "custom_plugin" / "plugin.py").write_text("PLUGIN = True\n", encoding="utf-8")
    (data_path / "web" / "password.hash").write_text(marker, encoding="utf-8")
    (data_path / "fonts" / "custom.ttf").write_text(marker, encoding="utf-8")
    (data_path / "auto_detect_state.json").write_text("{}", encoding="utf-8")
    (data_path / "venv" / "ignored.txt").write_text("ignore", encoding="utf-8")
    (data_path / "logs" / "ignored.log").write_text("ignore", encoding="utf-8")
    (data_path / "backups" / "old.kirara-backup.zip").write_text("ignore", encoding="utf-8")
