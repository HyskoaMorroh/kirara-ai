...
`plan` 只读且不修改文件，一次返回当前版本、候选 Python/npm/Git Tag、发布通道、远端和已占用版本；`--json` 是供 PowerShell、CI 和后续发布工具消费的稳定输出。

已占用版本分三个字段给出，**不能只看 `occupied`**：

| 字段 | 含义 | 看到它该做什么 |
| --- | --- | --- |
| `occupied` | 占用全集（碰撞判定用的就是它） | 候选一定不在其中 |
| `released` | 远端已有该 Tag，**已经发布过** | 接着往下找号 |
| `reserved_locally` | 只有本地有——一次**没推成功**的打标 | 先查那个号是不是上一次发布中断留下的残留 |

后两者的处置完全不同，而混成一个词的后果是版本号被无谓地跳过：一次失败的打标之后，每次重跑计划都会跳过那个号，而它其实从未发布，也没有任何发布产物与它对应。几次之后版本号里出现空洞，且没有任何地方记录那些号去哪了。`--local-only` 下 `released` 恒为空——那是显式声明「不查远端」，不是「远端没有」。`next` 保留为只输出候选版本的兼容命令。`bump` 会沿用当前发布通道递增编号，自动跳过已存在的本地与已解析远端 Tag，并在写文件前第二次查询 Tag、重新计算完整候选；如果并行发布抢先占用候选，或发布线已被推进到更高版本，它会停止并要求重新生成计划。`bump` 默认拒绝脏工作树，避免把用户未提交的修改混入版本同步；只有确认这些修改可以共存时才使用 `--allow-dirty`。`--kind stable` 用于把预发布转为正式版，`--kind minor` 和 `--kind major` 用于明确的功能线升级，不能让默认命令猜测这些边界。没有网络或只想在本地试算时，使用 `plan --local-only`、`next --local-only` 或 `bump --local-only`；不要把网络失败误当成“没有冲突”。
