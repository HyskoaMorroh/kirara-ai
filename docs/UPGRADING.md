...
本文用于把已有 Kirara AI 实例升级到 `3.3.0b8`。核心原则是：先保留可独立恢复的 A 数据副本，再让 B 读取副本；确认替代成功前，不删除旧程序、旧数据或备份。
...
4. 将整个 A 数据目录复制到独立、受限的位置，例如 `E:\backup\kirara-data-before-3.3.0b8`。不要用移动或覆盖原目录代替复制。
