...
适用于 `3.3.0b13` 的首次部署、模板选型、规则调试和日常维护。目标是让没有项目背景的使用者也能从空实例得到一条可验证的回复。
...
零节点时画布中央给出一段提示：从左侧节点面板拖入区块、把输出端口拖到输入端口连线、
`Ctrl+L` 自动排布、`Ctrl+Z` / `Ctrl+Y` 撤销重做（一次连续手势算一步）。

提示不拦交互（`pointer-events: none`），可以直接把节点拖到它上面。
这一点是刻意的：一个自己变成障碍的提示比没有提示更糟——用户照着它去拖，却拖不进去。

### 拖入节点的落点

落点先对齐到 20px 网格，再自动避开已有节点。两个曾经的缺陷已修：

- **候选耗尽时不再交出一个没检查过的坐标。** 原实现的循环先检查当前坐标，
  再算出下一步坐标进入下一轮，于是最后一轮算出的那对坐标从未被碰撞检查过却被直接返回。
  表现是画布上只看到一个节点、另一个被完全盖住，用户以为拖放没生效，再拖一次，叠三层。
  现在连硬上界都找不到空位时把落点推到所有节点的右下角之外——那里必然是空的。
- **坐标换算减掉了画布在视口里的偏移。** 原实现把鼠标的视口坐标直接交给一个
  假设「画布原点即视口原点」的换算函数。今天画布正好铺满视口，所以看不出问题；
  一旦画布被放进带侧栏或顶栏的容器，拖入的节点就整体偏移，而它不报错——
  节点确实生成了，只是位置不对，用户会以为自己没拖准。

### 打开工作流就看到节点重叠：两个来源

节点重叠有两个来源，处置不同：

1. **保存过的坐标本身重叠。** 载入时对已有坐标**不做任何重排**（那是设计决定：
   重排会把用户手工摆好的布局推翻）。而节点宽度区间在某个版本从 200–300px
   提升到 220–360px，旧版本保存的坐标在新宽度下就可能重叠；手工拖出来的重叠同理。
   处置是点一次「自动排布」（`Ctrl+L`），它用**实测**尺寸重排并保留连接关系。

2. **自动补位用的估算尺寸偏小。** 没有保存过坐标的节点（由工厂代码注册的那批：
   `chat:normal`、`chat:memory_store`、`game:*`、`system:*`）在打开时局部补位，
   而那一刻 DOM 还没渲染，尺寸 100% 来自估算。估算偏小会让布局算出「不重叠」
   而渲染出来重叠——用户只看到角标提示「建议点击自动排布」，而他刚刚点过。

   本轮修掉三处偏小，全在高度上：`.custom-node-body` 的 12px padding 是无条件
   渲染的（估算此前只在有配置项时算它，于是没有配置项的 `GetIMMessage` /
   `SendIMMessage` 这类头尾节点真实高度多 24px）；代码节点主体此前按定值 132px 计，
   现按预览行数算（封顶 6 行——不封顶会让一个 600 行脚本把画布拉散）；
   零端口代码节点渲染的是一段三行空态提示，而端口行数为 0 时它贡献 0。
   分隔是 60px，能吸收单侧误差，两侧叠加就会突破它。

### 连线拉过去又弹回来：现在会说明原因

连线被拒时会给出具体原因，而不再一律提示「类型不兼容」：

| 提示 | 实际含义 | 下一步 |
| --- | --- | --- |
| 该输入端口已有连线 | 一个输入只允许一条边 | 先删掉那条已有的线，或改接别的输入 |
| 端口类型不兼容 | 两端类型确实不匹配 | 换端口，或在中间加一个转换节点 |
| 源/目标端口不存在 | 端口已被改名或删除（改过配置的脚本节点最常见） | 重新配置端口，或重连到现有端口 |
| 连线缺少起点或终点 | 拖动没有落在端口上 | 松手时对准端口圆点 |

这四类此前**全部是静默的**：`isValidConnection` 判定不通过时 vue-flow 不会触发
`onConnect`，因此挂在那里的提示一次都没执行过。用户看到的只是线弹回来，
与「画布卡了」无法区分——而「该输入已有连线」和「类型不兼容」需要的动作完全相反。

松手在空白处不提示：那是改主意，不是错误。

### 自定义脚本节点为什么「一个端口都没有」

刚从节点面板拖进来的自定义脚本节点**两侧都是空的**，一条线也连不上。这不是故障，
也不是「断开的设计」——`CodeBlock` 的端口按实例声明，而不是按类型固定：
后端类级 `inputs` / `outputs` 是空字典，`/block/types` 只读类属性，
因此拖进来那一刻端口数确实是 0。节点内会直接说出来并指向配置面板。

端口在右侧配置面板「添加输入 / 添加输出」里声明，类型一律按 `Any` 参与连线校验
（前后端一致），所以**类型不兼容不会是脚本节点连不上的原因**。

一处相关缺陷本轮修掉：四个端口增删函数里有三个直接在 `data.config` 上原地赋值，
而历史快照是在写节点**之前**拍的——原地写入让快照里已经是新值，于是撤销拿回旧的
端口数组、`config` 里却停在新值上。后端端口正是从 `config` 读的，结果是画布显示
旧端口而保存下去是新端口。现四处写法一致：都先复制 `config` 再写。

### 撤销的粒度：一次操作 = 一次 Ctrl+Z

撤销栈按**用户感知的操作**记录检查点，而不是按内部写回节奏切片：

| 操作 | 需要按几次 Ctrl+Z |
| --- | --- |
| 拖动一个（或多个）节点到新位置，中途不松手 | 1 |
| 一键「自动排布」全图 | 1 |
| 复制选中的若干节点 | 1 |
| 在配置面板里连续修改同一个节点 | 1 |
| 在代码节点里连续输入 | 1 |
...
判据是鼠标与面板的起止，不是时间。此前有两处与这张表不符，都已修正：

- **一次连续拖拽会产生 6~7 个检查点**：历史抑制标记由 500ms 防抖回调清除，
  而拖拽还在继续，于是长拖拽被按时间切片。慢慢挪一个节点后想撤销，要连按七次。
- **批量操作（自动排布、复制多个节点）反而丢掉检查点**：批次内改的是画布自身的
  状态，写回工作流是防抖的，批次关闭那一刻「工作流数据」还没变化，比对结果是
  「无变化」→ 不压栈；而批次期间逐次记录也被抑制。两条路都不写历史，改动却在
  防抖到期后落库。后果比「多按几次」严重得多：撤销栈栈顶仍是**上一次**编辑的
  快照，一次 Ctrl+Z 直接毁掉那次编辑，且重做也回不来。

如果你在旧版本里养成了「排布完先手动保存再继续」的习惯，现在不需要了；
但保存本身仍然建议在一组编辑结束后做，因为预检只检查已提交的图。
