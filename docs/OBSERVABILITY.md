...
- **定价条目有两列身份：模型标识与显示名称，后者不参与匹配。**
  计价按 `(provider, model)` 找版本；`display_name` 只是标签。它存在的理由是
  价目表到几十条时 `claude-sonnet-5` 与 `claude-sonnet-5-20260514` 在一屏里
  只差一个后缀，而单价可能不同——挑出「我在用的那个」需要一个可读的抓手。
  三条边界：**不参与任何匹配**（拿它当键的话，改一个标签就会让历史账单换一个价格）、
  **`null` 与空串不同**（老价目文件没有这个字段，读进来照旧可用，
  显示时回落到模型标识而不是留空白）、**进 `integrity` 摘要**（不进摘要的话，
  一次手工改标签会让文件与摘要不一致而无人发现，随后真实篡改也检测不出来）。
  上游同步会带上目录里的 `name`，但**不回落到 provider 的 `name`**——
  那会让同一家所有模型都显示成「Anthropic」，比没有标签更容易读错；
  且只有标签变化不计入价格变化，否则每轮同步都重写会推高 revision，
  让乐观锁误判成有人在并发改价。
...
请求日志表逐条给出四类 Token 的**拆分**（输入 / 输出 / 缓存命中 / 缓存创建）
以及合计。只给合计是歧义而不是省略：同样 100 万 Token，一家几乎全在读上下文、
另一家几乎全在生成，成本能差 5~10 倍（输出单价通常是输入的数倍，缓存读取又比
输入便宜一个量级）。要回答「这条为什么这么贵」，合计恰恰是唯一回答不了的数字。
缓存两列显示「未上报」而不是 0——`null` 是没有任何上游报过缓存维度，
`0` 是报了、确实没命中；混成一个会让人去排查一个并不存在的缓存失效问题。
表格总宽超过常见视口，因此声明了 `scroll-x`：不声明时 naive-ui 会按容器宽度
...
| `PUT /backend-api/api/llm/resilience/queue` | 重排一条模型的故障转移队列（创建者身份，一次给出整条次序） |
...
`PUT /backend-api/api/llm/resilience/queue`（创建者身份，请求体
`{"model": "<模型 id>", "providers": ["<按新次序排列的全部供应商>"]}`）
一次提交整条队列的次序。

**为什么不是「把某一家改成某个优先级数字」。** 逐个改会经过中间态：把 P3 改成 1 的
那一刻队列里出现两个 1，而相等优先级的相对次序由 `active_backends` 的列表下标决定
（`get_provider_candidates` 的第二排序键），也就是用户看不见的东西。整条提交没有
这个窗口。

三条边界，都会以 400 拒绝而不是尽力而为：

| 情形 | 为什么必须拒绝 |
| --- | --- |
| `providers` 缺了这条队列里的某一家 | 它落在哪里取决于它原本的数字，而提交者以为自己排的是整条队列 |
| 出现队列外的名字 | 通常是拼错；接受它等于悄悄改动另一条队列 |
| 同一家出现两次 | 「它排第几」没有答案，而只查长度会被这种输入蒙过去 |

模型名不存在时返回 404：拼错的模型名若回 200，「排序成功」与「什么都没发生」
看起来一模一样。

**只交换已在用的数字，不发明新数字。** 同一家供应商可以同时服务多个模型；
凭空抬高它的 `priority` 会连带改动另一条此刻不在屏幕上的队列。
原本相等的数字会被拆成严格递增的一组——全是默认 `100` 时，多重集重排是恒等变换：
保存成功而次序没变。

落盘失败时内存里的次序退回原值。不退等于「面板显示新次序、重启后回到旧次序」，
而这个差异没有任何地方会报出来。

#### 改过的熔断参数立刻生效

五个 `circuit_*` 参数（失败阈值、错误率阈值、最小请求数、恢复等待时间、
恢复成功阈值）此前只在熔断器**被创建的那一刻**被读进去。`PUT /llm/backends/<name>`
的 unload / load 两个分支各有条件：未加载的后端不会被 unload，停用的后端不会被 load。
编辑一个 `enable=False` 的后端时两者都不成立——参数保存成功，而下一次故障仍按旧阈值
熔断。这个后端也不出现在 `resilience/status` 的行里（那里只遍历 `active_backends`），
所以连「重置熔断」这个变通入口都没有。

现在保存后无条件刷新一次，且**刷新阈值而不重建熔断器**：当前状态（是否熔断）、
在途计数、已积累的成败样本与迁移历史全部保留。重建等于每次编辑配置都取消一次
正在生效的隔离——改的是超时数字，被改掉的是「这家已经被隔离」，那比参数晚生效更糟。

一个连带约束：样本窗口跟着 `circuit_min_requests` 走。窗口比它小时，
错误率那一支的判断（`len(outcomes) >= min_requests`）永假，配置里的错误率阈值
被静默关掉，只剩连续失败阈值起作用。收窄窗口时保留**最近**的样本而不是最早的——
健康判断看的是当下。
