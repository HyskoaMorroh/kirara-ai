    ...
    async def _execute_normal_block(self, block: Block, executor, loop):
        ...
        for future, block in futures:
            try:
                ...
                # 针对 HTTP 错误提供更详细的错误信息
                if e.response is not None and e.response.status_code == 524:
                    error_msg = (
                        f"Block {block.name} execution failed: HTTP 524 Timeout Error. "
                        f"The origin server took too long to respond. "
                        f"This may be caused by an overloaded server or long-running task."
                    )
                    self.logger.error(error_msg)
                    raise BlockExecutionFailedException(error_msg) from e
                else:
