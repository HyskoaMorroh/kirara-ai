...
**[可靠性清单](#-可靠性与可观测性)**
· [HTTP API](#-http-api)
· [QQ 专项](docs/QQ_ONEBOT_OPERATIONS.md)
...
> 目标：从零到**收到第一条 AI 回复**。每一步都给判据，不靠「看起来成了」。

| 步骤 | 动作 | 怎么确认这步成了 |
|:--:|---|---|
| **1** | 复制 `.env.example` 为 `.env`，填 `DOCKERHUB_IMAGE` | `docker compose config` 不报错 |
| **2** | `docker compose up -d` | `docker compose logs --tail=50` 里出现监听端口 |
| **3** | 打开 `http://<你的地址>:8759`，设定登录密码 | 能进 WebUI |
| **4** | 「模型」页添加供应商：适配器 + API Key + 至少一个模型名 | 「快速开始」页自检里 `llm_backends_available` 为 `pass` |
| **5** | **重启一次**服务 | `GET /agents` 至少返回一条，且 `model_priority` 非空 |
| **6** | 「聊天平台」添加并启用一个 IM 适配器 | 适配器详情页状态为**已连接**（QQ 冷启动期间是「正在启动」，属正常） |
| **7** | 从该平台发一条消息 | 收到回复 |

第 5 步的重启不是仪式：注册表为空时启动会自建 `default-agent` 并绑定提示词与记忆。
**没有它，六个适配器全部要求解析到 Agent 才回复**——一个刚起来的实例在你手动建出
第一个 Agent 前，任何渠道都不回话。已经有 Agent 时启动不会碰它。

判据都读**服务端自检**而不是界面绿勾：

```bash
curl -s -H "Authorization: Bearer <token>"   http://127.0.0.1:8759/backend-api/api/system/readiness | python3 -m json.tool
```

> 读不到这个接口时引导页会说「暂时读不到自检结果」，而**不会**把任何一步标成已完成。
> 「不知道」与「未就绪」是两件事——把前者显示成后者，会让人去修一个不存在的问题。

完整落地路线见 **[`docs/PRACTICAL_PLAN_AND_TUTORIAL.md`](docs/PRACTICAL_PLAN_AND_TUTORIAL.md)**。

***

## 🗺 系统结构：一条消息怎么变成回复

理解这张图就能定位任何一次故障发生在哪一层。

```
  ┌─────────┐   ┌─────────┐   ┌──────────┐   ┌─────────┐
  │   QQ    │   │ Telegram│   │ 企业微信 │   │  WebUI  │   ← 渠道
  │ OneBot  │   │         │   │ 公众号   │   │  HTTP   │
  └────┬────┘   └────┬────┘   └────┬─────┘   └────┬────┘
       └─────────────┴─────────────┴──────────────┘
                          │  统一入站：去重收据（同一条上游消息只触发一次）
                          ▼
                  ┌───────────────┐
                  │  调度规则     │  优先级 100 系统指令 / 60 命令 / 30 聊天 / 0 兜底
                  └───────┬───────┘
                          ▼
                  ┌───────────────┐
                  │  渠道身份     │  会话 > 账号 > 渠道 > 全局默认
                  │      ↓        │
                  │    Agent      │  ← 全新部署由启动兜底建出 default-agent
                  └───────┬───────┘
                          ▼
        ┌─────────────────┼─────────────────┐
        ▼                 ▼                 ▼
  ┌──────────┐    ┌──────────────┐   ┌────────────┐
  │ 模型链   │    │ Prompt/Skill │   │  MCP/Hook  │
  │ P1→P2→…  │    │   Memory     │   │  工具白名单│
  └────┬─────┘    └──────────────┘   └────────────┘
       │  故障转移：只对「可重试」错误换家；参数类错误改一处再试
       ▼
  ┌───────────────┐
  │  上游模型     │  熔断 closed/open/half-open · 限额余量 · 首字节/静默超时
  └───────┬───────┘
          ▼
  ┌───────────────┐
  │  统一排版     │  解析共享 · 符号表按平台各一份 · 长回复「第 N 页 / 共 M 页」
  └───────┬───────┘
          ▼
  ┌───────────────┐
  │  投递队列     │  按页独立投递 · 结果未知的隔离不重发 · 指数退避有上限
  └───────┬───────┘
          ▼
       用户收到
```

**每一层都会落一条时间戳**：收到事件 → 工作流开始 → 模型首字节 → 模型完成 →
排版完成 → 发送开始 → 发送结果。因此「为什么慢」是一个可回答的问题，
而不是一句「QQ 慢」。未测到的阶段保持空值，不用 0 冒充。
...
三个主要工作面。每张图下面一句话说明**你在这里能决定什么**——
不是「这里有什么」，而是「哪些决定只能在这一页做」。
...
> 供应商、模型清单、故障转移队列次序与五个熔断参数都在这里。
> **队列次序在看到队列的地方改**（每行上移/下移一次提交整条次序），
> 而不是回到表单里算一个中间的优先级数字。
...
> 画布决定「一条消息经过哪些节点」。自动布局是确定性的，
> **已经由你编辑过的坐标不会在后台刷新时被覆盖**；
> 只有从未保存过坐标的节点才会被补位，并且会明确告诉你补了几个。
...
> Skill / Hook / MCP / Prompt 的安装、版本回退与卸载。
> **「停用」不是卸载**——停用不释放磁盘、不清注册表，那个 ID 也不会重新可用。
...
- 图片发送
- 语音回复
- Stable Diffusion、Flux、Midjourney 等绘图模型
...
**支持的大模型**：支持 OpenAI、DeepSeek、Claude、Gemini、Qwen、Mistral、豆包、Minimax、Kimi、Mistral 等主流大模型
（支持 QQ、Telegram、Discord、微信 等聊天平台，逐平台能力见下表）。

### 🛡 可靠性与可观测性

下面每一条都对应一个**真实发生过的失败**，而不是一项功能声明。
先看速览表定位，需要理由时展开详情——那些理由是「为什么这样做」的证据，
也是判断某个行为是不是缺陷的依据。

#### 🟦 渠道接入与连接状态

> 回答「机器人到底连上没有」。九种状态里只有**两个**不需要动手，其余七个各自指向不同的处置——把它们合成一个「未连接」，就等于让操作者去猜。

| 能力 | 解决什么 |
|---|---|
| **内置 OneBot V11 / QQ 适配器** | 九种连接状态 + 固定原因码，区分「等就行」与「去修」 |
| **扫码登录生命周期可查** | 10 种状态、有效期倒计时，过期由时钟判定而不等上游日志 |
| **通知与请求事件不再静默丢弃** | 被踢出群、被禁言会留下日志；好友申请不自动同意 |

<details>
<summary>展开这一组的完整说明与理由</summary>
...
> 回答「用户收到的和模型生成的是不是同一份内容」。解析共享、符号表按平台各出一份，差异只允许存在于渲染层。
...
| 能力 | 解决什么 |
|---|---|
| **统一排版管线** | QQ / Telegram / 企业微信共用一套结构化渲染，不各写一套 Markdown 解析 |
| **代码复制按各渠道能力落地** | 有原语就给真按钮，没有就让代码单独成条——不画点不动的按钮 |
| **发送节流有总额上界** | 多页回复的页间等待按频率计费而非按字数累加，默认上界 6 秒 |
| **持久化投递队列** | 按页独立投递，结果未知的隔离**不重发**，明确失败才有限重试 |
| **入站去重** | 同一条上游消息只触发一次工作流，避免重复计费与重复回复 |
...
> 回答「插件真的参与了这次对话吗」。关系模型是**渠道身份 → Agent → 模型链 → Prompt / Skill / Memory / MCP**，四个入口走同一条链。

</details>

#### 🟨 Agent、技能与插件

> 回答「插件真的参与了这次对话吗」。关系模型是 **渠道身份 → Agent → 模型链 → Prompt / Skill / Memory / MCP**，四个入口走同一条链，不需要在界面上挑任何插件。

| 能力 | 解决什么 |
|---|---|
| **装好就能回话** | 注册表为空时启动自建 `default-agent`；模型链从已配供应商拉候选 |
| **四个渠道同一套失败说明** | 按「接下来该做什么」分类；配置缺失不说「稍后再试」 |
| **Tool Search：工具也走渐进披露** | 工具超过 12 个时只放目录，完整 schema 按需取回 |
| **Teammates：Agent 之间可委派** | 队友用自己的模型链执行子任务；委派深度有上限，自委派被拒 |
| **开箱即有 36 条插件** | 31 个技能 + 5 个角色提示词随镜像分发，**安装不出网**；12 个 MCP 模板已登记 |
| **受控扩展** | Skill / Hook / MCP / Prompt 带 manifest、版本、来源、权限边界与审计 |
| **提示词能从界面直接写** | 提示词 / 记忆 / 会话可填表单创建；能起进程的三类仍走上传 + 确认 |
| **资源装得进也退得出** | 版本回退（内容还在）与卸载（删前备份、释放 ID） |
| **缺什么依赖说得出来** | 点名缺的是哪个命令；「没探测过」与「确实没有」分开显示 |
| **依赖可见可控** | Node / uv / agent-browser 等的就绪状态，有真实安装器的可受控安装 |
| **创建者身份可延伸到 IM 渠道，且能在界面上声明** | 渠道与发送者标识一起比对；群聊默认不生效 |

<details>
<summary>展开这一组的完整说明与理由</summary>

* [x] **装好就能回话**：注册表为空时启动自建一个 `default-agent` 并绑定提示词与记忆。
  没有它，六个适配器全部要求解析到 Agent 才回复，一个刚 `up -d` 起来的实例在用户手动
  建出第一个 Agent 前**任何渠道都不回话**。三条边界：注册表非空一律不动；没有具备
  Chat 能力位的模型时不建（否则把「你还没配模型」换成一次运行时解析失败）；
  不绑 MCP 与 Hook（那两类会拉起进程）。「Agent 管理」里的模型链与 Provider 白名单
  从已配供应商**拉取下拉候选**，不必手打模型 ID——拼错不会当场报错，
  会等到某次真实对话才失败；同时仍允许填候选外的名字（只提示不拦保存），
  好让「先配 Agent 再配供应商」这个顺序成立
* [x] **四个渠道同一套失败说明**：同一个失败此前在 Telegram 是一句英文、在企业微信是
  一套中文分类、在 QQ 与 OneBot **只记日志**（用户那侧完全静默，无法区分「机器人挂了」
  与「我的消息没发出去」，只会反复重发）。现共用一份描述，按「用户接下来该做什么」
  分类，**配置缺失不说「稍后再试」**——重试永远不会成功；原始异常文本截断后保留供检索
...
* [x] **Teammates：Agent 之间可委派**：模型获得 `delegate_to_<id>` 工具，
  ...
  这种每层都真实花钱的无限递归
* [x] **开箱即有 36 条插件，且预置不等于放权**：31 个纯文档技能与 5 个角色提示词
  随 wheel 与镜像分发，`ensure_builtins()` 在**完全离线**的环境里也能装上
  （实测 51 条内置离线可装、2.1 秒）。只收纯文档技能是因为运行时镜像只装了
  Python、ffmpeg 与 libmagic1——带 `.sh` / `.js` / `.py` 的技能装上后界面显示正常、
  一使用就失败，比不装更糟；本机那 135MB 里有 127MB 属于这一类。
  两条 MCP（`node_repl` / `context-mode`）故意不收：命令是本机绝对路径，
  在别人的机器上是死配置而界面上与其他模板同形。
  **权限边界在预置之后仍然拦得住**：随包资源一条都不许声明能改服务器的权限，
  默认 `creator_channel_identities` 为空，没有 principal 时门禁一律拒绝——
  也就是 IM 渠道默认拿不到能操作 VPS 的工具，白名单里的创建者才拿得到
* [x] **受控扩展**：Skill / Hook / MCP / Prompt 带 manifest、版本、来源、权限边界与审计，
  Hook 支持按工具名匹配、按事件启停与上线前预演
* [x] **提示词能从界面直接写**：提示词、记忆与会话这三类的全部内容就是正文，
  因此可以填一个表单就创建（打包与摘要在服务器侧完成，落盘后与内置条目同形）。
  Skill / Hook / MCP 不在这条路上——它们的正文会被当作行为说明执行、
  是能起进程的命令声明、或启用即拉进程，那三类继续走「打包 + 上传 + 显式确认」。
  改正文保存为**新版本**而不是就地改文件：`content_sha256` 把清单与文件绑在一起，
...
* [x] **依赖可见可控**：Skill 包与它依赖的可执行程序是两件事；依赖目录列出
  Node / uv / agent-browser / graphify / rtk / memsearch / context-mode 等的就绪状态，
  有真实安装器的可受控安装（只跑服务器登记的固定命令）；
  装不了的那一条（`caveman`：公共 npm 上是同名的模板引擎，真包为 404）
  只探测不代装，并在指引里写明两个名字各是什么——猜一条安装命令会装上错的那个，
  命令存在、探测通过、功能不对，比显示「未安装」更糟
...
> 回答「一家上游坏了会怎样」。关键区分是**可重试**与**换一家也没用**——后者靠整流改一处再试，而不是把故障转移队列打满。
...
| 能力 | 解决什么 |
|---|---|
| **多 Provider 故障转移** | 优先级队列 + 三态熔断（跨重启保留）+ 手动重置 + 面板内改队列次序 |
| **供应商级请求策略** | 推理强度四档、移除 AI 自我署名、禁用版本探测——逐供应商生效 |
| **请求整流器** | 五类「不改就一定失败」的情形改一处再试一次；换供应商也没用 |
| **上游限额余量可见** | 从限额头读「离上限还有多远」；未上报显示「未上报」而不是 0 |
| **流式、非流式与真流式三档** | 按 Agent / 渠道 / 进程三层配置；不支持的渠道自动退回 `aggregate` |
| **四家上游都实现流式** | 各自解析自家帧格式；上游没给用量就保持未知而不补 0 |
...
* [x] **多 Provider 故障转移**：优先级队列、可重试错误分类（认证/参数/内容策略不重试）、
  ...
  真实流量）、只影响那一家。**队列次序也在同一个面板上改**：每行的上移/下移按钮
  一次提交整条队列的新次序，而不是让人回到供应商表单里算一个中间的优先级数字
  （那会经过两家同优先级的中间态，此时相对次序由用户看不见的列表下标决定）。
  五个熔断参数改完立刻生效，不必重启——包括编辑一个当前停用的后端。
  总时间预算与三档取回方式也可在
...
* [x] **请求整流器**：上游因参数约束**拒绝**时改一处再重试一次，而不是把一次
  必然失败原样抛给用户。修五类「不改就一定失败、原因又不在错误里」的情形：
  思考预算与最大输出长度关系不对、换模型后失效的思考签名、不支持图片的模型
  收到图片、模型压根不支持思考、上游不认识 `reasoning_effort` 字段。
  这几类**换供应商也没用**——
  ...
  **四种请求体形状（Claude、十个 OpenAI 兼容适配器、Gemini、Ollama）各按自己的
  字段整流**：规则按载荷形状分派，而不是把一家的字段名写进另一家的请求体——
  往 Gemini 注入 Anthropic 的 `thinking` / `max_tokens` 会让它直接
  400 INVALID_ARGUMENT，真正的预算位反而没改，等于把可重试的错误变成必然失败。
  某家没有对应位置的规则在那家**不出现**（Gemini 不回传思考签名、Ollama 的思考
  只有开关没有预算），而不是空转出一个「试过了但没改成」
...
> 回答「钱花在哪、慢在哪」。所有数字都标明来源，估算与未知从不冒充真实用量。
...
| 能力 | 解决什么 |
|---|---|
| **真实成本统计** | 按当时价格快照计费，区分四类用量来源；「只报了一部分」单独标注 |
| **价目自动同步** | 手工改过的价格不被覆盖；单位不做换算（乘一次就是千倍偏差） |
| **端到端投递时间线** | 七个阶段逐段计时落库，另有跨渠道对比表 |
| **模型目录定期刷新有界面** | 间隔天数、上次成功、下一轮预计时刻；空值显示「—」而不编时间 |
| **前端构建过期会被 readiness 报出来** | 静态目录与后端版本一起报出，不一致判 warn |
| **会话可管理** | 列出持久化会话与待确认队列，清空单个会话历史 |
...
> 关系模型是**渠道身份 → Agent → 模型链 → Prompt / Skill / Memory / MCP**。
> 四个入口（WebUI / QQ / Telegram / 企业微信）走的是同一条链，**不需要在界面上挑任何插件**。
> 详见 [`docs/AGENTS_SKILLS_HOOKS_MCP_GUIDE.md`](docs/AGENTS_SKILLS_HOOKS_MCP_GUIDE.md)。
...
两个体检脚本用来找**成片同形**的缺陷，而不是一处一处踩：

```bash
# 后端返回的键里，哪些前端一个字都没声明或读过
.venv-win/Scripts/python.exe scripts/audit_api_field_coverage.py

# 哪些测试只 grep 源码文本——那种断言在行为坏掉时照样绿
.venv-win/Scripts/python.exe scripts/audit_source_grep_tests.py

# 哪些后端路由没有任何前端调用点——「建好了但界面到不了」
.venv-win/Scripts/python.exe scripts/audit_unreachable_routes.py
```

三者都是**线索生成器而非判决**：它们解析不了经过辅助函数的取值，
因此会多报（错误分支里的键、只在服务端用的键）也会少报（在 helper 里拼出的响应）。
每一条命中都要读过代码才算缺陷。

第二个脚本的结论由一条护栏钉住：`tests/test_grep_only_tests_have_behaviour.py`
要求每个「整个文件只 grep 源码」的测试都在表里登记一份同题的行为测试。
新写一个这样的文件会让它红，并要求回答「这一页的行为由谁验证」——
答案可以是「后端已覆盖」，但必须写下来。源码 grep 本身不禁止：
import 清单、`data-test` 钩子、逐字文案、「不该出现某段代码」这类否定断言
确实只活在源码文本里。
...
`pyproject.toml` 是**唯一版本源**。Python、npm、`uv.lock`、Docker、WebUI、
GitHub Actions、Tag、Release 文案与文档里的当前版本都由它推导，任何一处都不许硬编码。

| 子命令 | 回答什么 | 何时用 |
|---|---|---|
| `plan` | 当前版本、候选 Python/npm/Tag、通道、远端、已占用版本 | 发版前先看一眼（`--json` 供 CI） |
| `next` | 只输出候选版本号 | 脚本里取一个值 |
| `bump` | 把候选写进全部版本载体 | 确认候选无误后 |
| `check` | 各处版本是否一致 | 每次改完、每次发版前 |
| `verify-tag` | Tag、HEAD、期望 commit、远端 Tag 对象是否同一个 | **创建 Release / 构建镜像前必须过** |
| `discover` | 列出全部版本载体 | 新增了带版本号的文件时 |

常规发布不需要手工猜测或填写下一版本号，使用版本工具先预览、再同步：
...
<details>
<summary>为什么镜像里还要写 <code>org.opencontainers.image.revision</code></summary>
...
<details>
<summary>为什么 TLS 不够、摘要缺失时为什么拒绝安装</summary>
...
<details>
<summary>PEP 440 与 semver 在这一点上结论相反</summary>
...
| `PUT /llm/resilience/queue` | 重排一条模型的故障转移队列：一次给出整条次序，需创建者身份 |
...
| `GET /agents/sessions` | 持久化会话列表（条数、时间戳与渠道身份，无对话正文） |
...
| `GET /resources?query=` | 按名称、ID、描述与**正文**检索已安装资源（正文只读 prompt / memory / session；响应不含正文） |
...
| `PUT /resources/<id>/runtime` | 受管 MCP 资源在本机怎么跑：追加启动参数（文件类服务器的可访问目录）、环境变量、`roots`、工作目录与启动超时。命令与传输类型来自已签名的资源包，不可改 |
