...
比较基线为 `3.2.0`，比较目标为 `3.3.0b18`。本文件记录源码与发布行为的对应关系；实际发布状态以 GitHub 和镜像仓库为准。
...
- **模型链里 `claude-fable-5-1` 会让整个 opus 家族消失（需求 8）**：用户指出界面上的
  模型优先链与规则不符。截图本身是**未点「按规则重排」的原始顺序**，那部分正常；
  但把截图那 19 项真的跑一遍重排，发现 `claude-opus-5`、`CHMA/claude-opus-5`、
  `claude-opus-5-thinking` 三条全部被淘汰，只剩一条 `ANT/claude-fable-5-1`。

  根因是一处**字面无法消解的歧义**：`claude-fable-5-1` 与 `claude-opus-4-6`
  同为 `<家族>-<档位>-<数字>-<数字>`，但前者 `-1` 是变体序号（系列仍是 5）、
  后者 `-6` 是次版本（4.6）。原实现把两者都读成小数，于是 5.1 > 5，
  opus 全族被当成旧系列。这个错不抛异常，只是让最强的模型不参与故障转移。

  第一次的修法是「个位数一律读主版本」，被既有测试当场否掉——那会让
  `-3-5` 与 `-3.5` 不再等效，而那是需求明确要求的。这个失败说明判据不能只看字面。

  最终改为**两遍扫描**：先收集全族**无歧义**的系列号（点号形式 `5.6`、
  数字结尾形式 `opus-5`），再据此消解歧义——若歧义名字的主版本已是该族公认系列，
  后面那个数字就是变体。`claude` 族里 `opus-5` 确认了 `5` 是系列，
  因此 `fable-5-1` 判为变体；而只有 `gemini-3-1-pro` 一种写法时 `3` 不在确认集合里，
  仍读作 3.1，`-3-1` 与 `-3.1` 的等效性得以保留。这也符合「跟着刷新出来的模型
  动态调整」——`5` 是不是系列取决于这批模型里有没有别的 `*-5`，而不取决于写法。

  5 条回归测试，其中一条直接用那 19 项真实链作输入，断言 opus 必须排在 claude 段最前。


- **11 处 loguru 日志的参数被静默丢弃（需求 14）**：真实容器冷启动的日志里出现

      WARNING | ResourceCatalog | 内置资源 %s 预置失败，已跳过（不影响启动）：%s

  两个值一个都没显示。loguru 用 `{}` 占位符，写成 `%s` 时参数不会被替换。
  这条日志的全部作用就是说出「哪一条内置资源没装上、为什么」——而它在那次启动里
  **确实触发了**（断网时 `skill:agent-browser` 需要出网因此被跳过），
  于是那个事实无从追查，只能另写代码调 `builtin_provisioning_report()` 才查得到。

  最初 grep `"%s` / `%s"` 只找到 3 处，改用 AST 扫描才找齐 **11 处**：占位符在
  字符串中间、参数换行的写法被 grep 漏掉了。分布是 MCP 服务器 2 处、
  四个 LLM 适配器各 2 处、认证服务 1 处。适配器那 8 处记的是
  「整流器改写了哪一处、细节是什么」，参数丢了之后一次成功的整流在日志里
  看不出改了什么。

  新增 `tests/test_loguru_placeholders.py`（7 项）。判据是**扫描整棵源码树**
  而不是逐个文件补断言——本轮已经在两个不同文件里踩过同一形状，逐文件的写法
  必然漏掉下一个新文件。门禁自身也要能自证：一条用例喂已知坏样本进去，
  检测器必须抓到，否则「只会返回空列表的检测器」与「代码是干净的」
  在断言上无法区分。同时排除两类正确写法：`"%s" % value` 预格式化、
  以及不带参数的消息（那通常是在讲解格式）。

  写测试时踩到第二层：用 pytest 的 `caplog` 断言 loguru 输出，拿到的是空串
  ——loguru 不经过 stdlib logging。那条用例在**任何**实现下都会失败，
  包括正确的实现。改用 `logger.add()` 挂临时 sink 直接收字符串。

- **`package-data` 的随包资源清单不完整（需求 23.3）**：wheel 里那 5 个随包提示词
  能装齐，靠的是 `include_package_data = true` 让 `MANIFEST.in` 的递归包含也对
  wheel 生效——而 `package-data` 只列了 `skills/**/*`。两条规则各自不完整、
  互相兜着，改动任一条都会让提示词在 pip 安装的部署上静默消失，
  而 Docker 走 `COPY . .` 不经过 wheel 打包因此永远不暴露。
  现补全为四项（两个索引 + 两类正文），两条路径各自独立成立。
